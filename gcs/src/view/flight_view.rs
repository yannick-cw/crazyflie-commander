...
/// step with the occupancy grid's extent in `mission_computer`, otherwise the map shows blank
...
pub fn view(model: &Model, frame: &mut Frame) {
    ...
    match &model.state {
        State::ManualControl(s) => frame.render_widget(manual_control_bar(s), mission_area),
    ...
    if let State::ManualControl(s) = &model.state
        && s.is_recording
    {
...
fn mission_bar(mission: Option<&mission_monitor::Model>) -> Gauge<'static> {
...
fn recording_panel(s: &manual_control::Model) -> Paragraph<'static> {
    Paragraph::new(vec![
        Line::from(vec![
            Span::styled(
                " ● ",
...
fn manual_control_bar(s: &manual_control::Model) -> Gauge<'static> {
...
fn map(
    t: &Telemetry,
    mission: Option<&mission_monitor::Model>,
...
fn mission_path(mission: &[MissionItem]) -> Vec<PathElem> {
    let m = |v: &Meters| v.0 as f64;
    let mut cursor = (0.0, 0.0);
    let mut elems = Vec::new();
    for cmd in mission {
        match cmd {
            MissionItem::Move { x, y, .. } => {
                let next = (cursor.0 + m(x), cursor.1 + m(y));
                elems.push(PathElem::Seg(cursor, next));
                cursor = next;
            }
            MissionItem::MoveToWaypoint { x, y, .. } => {
                let next = (m(x), m(y));
                elems.push(PathElem::Seg(cursor, next));
                cursor = next;
            }
            MissionItem::SmoothPath { waypoints, .. } => {
                for w in waypoints {
            ...
            MissionItem::Orbit { radius, .. } => elems.push(PathElem::Ring(cursor, m(radius))),
            MissionItem::BilliardBox(p) => {
...
fn waypoints(mission: &[MissionItem]) -> Vec<(f64, f64)> {
    let m = |v: &Meters| v.0 as f64;
    mission
        .iter()
        .scan((0.0, 0.0), |cursor, cmd| {
            *cursor = match cmd {
                MissionItem::Move { x, y, .. } => (cursor.0 + m(x), cursor.1 + m(y)),
                MissionItem::MoveToWaypoint { x, y, .. } => (m(x), m(y)),
                MissionItem::SmoothPath { waypoints, .. } => waypoints
                    .last()
                    .map(|w| (m(&w.x), m(&w.y)))
                    .unwrap_or(*cursor),
                MissionItem::BilliardBox(p) => (
...
fn range_line(label: &str, m: Meters) -> Line<'static> {
    ...
    /// Matches the avoidance threshold in `mission_computer`.
