use crate::pages::home::ModeSelection::{ManualControl, MissionPlan, MissionSelect};
...
pub fn view(model: &Model, supports_enhancements: bool, frame: &mut Frame) {
    ...
    let modes = [
        ...
        (ManualControl, "Free Flight"),
    ];

    let mut lines: Vec<Line> = modes
        .iter()
        .map(|&(ref mode, name)| {
            if *mode == ManualControl && !supports_enhancements {
    ...
    let description = if model.selected_mode == ManualControl && !supports_enhancements {
        ...
        match model.selected_mode {
            ...
            ManualControl => "manual, observe-only flight", // TODO: observe only for now
        }
    };
