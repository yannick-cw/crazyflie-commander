...
use crate::control::command_unit::MissionItem::{
    BilliardBox, Land, MoveToWaypoint, Orbit, SmoothPath, Takeoff,
};
use crate::control::command_unit::{
    BilliardParams, Meters, MetersPerSecond, MissionItem, Waypoint,
};
...
pub fn smooth_curves() -> Vec<MissionItem> {
    let one_loop = [
        Waypoint {
            x: Meters(0.5),
            y: Meters(0.0),
...
pub fn body_frame_smooth() -> Vec<MissionItem> {
    let one_loop = [
        Waypoint {
            x: Meters(0.75),
            y: Meters(0.0),
...
pub fn lawn_mower() -> Vec<MissionItem> {
...
pub fn billiard_box() -> Vec<MissionItem> {
